...
## Current state (June 2026) — `docs/STATS.md` is the single source of truth for
all project-total numbers (properties on spine, raw artifact counts, court-case
counts, legal-grade count, grant rates). It is generated by
`scripts/187_generate_stats.py` — re-run it after any load script and commit the
diff. **Do not hardcode a project-total figure in this file or any other doc;
link to STATS.md instead.** Full narrative review: `docs/progress_report_2026-06.md`.

The project is a **multi-source evidentiary spine** loaded to PostgreSQL/PostGIS,
not a single court scraper. The occupation court layer was expanded June 2026
from the original 4 Mariupol district courts to **all 26 DNR
original-jurisdiction courts that returned ownerless-property records**
(`scripts/178`/`182`–`185`). Of the 15 enabled courts that returned zero under
their own domain, a ВС ДНР venue notice (captured 2026-06-28) shows **10 had
their jurisdiction formally transferred** to an absorbing court already in the
dataset — not truly zero-activity, just filed elsewhere (`scripts/186` recovers
confirmed Avdiivka cases hidden inside Yasynuvata's docket this way); the other
**5 are genuine building-relocated courts** with no jurisdiction transfer, split
between destroyed Russian-held cities (Bakhmut, Vuhledar) and “ghost” courts for
Ukrainian-held territory Russia claims on paper (Sloviansk, Kramatorsk,
Pokrovsk). At first instance, the **grant rate is the base rate the
self-selected appellate layer hides** — most petitions are simply granted, and
a meaningful share of grants name a living owner in the case file (knowing
dispossession; see `docs/dnr_district_first_instance_2026-06.md` for current
figures). New district cases are reconciled against the original Mariupol
harvest before loading (`scripts/184`) so overlapping cases are never
double-counted. The **Mariupol court conveyor shut down first** (filings/
decisions → 0 by mid-2026) while the rest of DNR keeps running — the data
signature of ФКЗ-4 (Dec 2025) abolishing the court stage, Mariupol being the
Roskadastr pilot. The live front line has moved to the ownerless registry ahead
of the **1 July 2026** re-registration deadline. Appellate (ВС ДНР) and
cassation (2kas) layers are also captured. **100% of published rulings redact
the street address (`<адрес>`) at source** — standard nationwide Russian
court-records depersonalization (262-ФЗ), not occupation-specific; the
practical effect on displaced owners is what's specific to this dataset.
Gaps & follow-ups: progress report §5.
