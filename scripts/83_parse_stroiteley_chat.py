#!/usr/bin/env python3
"""Parse captured "Строителей 175,177,171,163,166,152" Telegram messages.

Reads raw message JSON captured by script 82. Six buildings on the spine —
all title-stripping track (ownerless registry), NOT demolished.
Same boulevard as Case 7 (74-88 demolished+built-over).

Chat active from 2022-03-13 (siege onset) → 2023-08-31.

Key evidentiary targets:
  - Siege-era presence (rebuts abandonment for ownerless predicate)
  - OWNERLESS process mentions (бесхозяйн, муниципальная собственность)
  - Sealing events (пломб, опечатал)
  - Official notices (уведомления, акты, Кольцов decrees)
  - Construction on adjacent demolished plots (same Строителей boulevard)
  - Apartment number refs → cross-reference with registry_inclusion events

Building numbers in regex: 152, 163, 166, 171, 175, 177.

Outputs:
  data/parsed/stroiteley_chat_signals.jsonl
  Console summary

Run:
    python scripts/83_parse_stroiteley_chat.py
"""
import json
import logging
import re
...
OUT          = ROOT / "data" / "parsed" / "stroiteley_chat_signals.jsonl"
...
BLDG_NUM = re.compile(r"\b(?:д\.?\s*|дом\s*)(152|163|166|171|175|177)\b", re.I)
...
def main() -> None:
    ...
    # ── Summary ───────────────────────────────────────────────────────────────
    print(f"\n{'='*70}")
    print(f"Строителей 152/163/166/171/175/177  —  {len(rows)} messages parsed")
    print(f"{'='*70}")
    ...
    for label, flag in [
        ...
        ("NEW BUILD (same boulevard)", "new_build"),
