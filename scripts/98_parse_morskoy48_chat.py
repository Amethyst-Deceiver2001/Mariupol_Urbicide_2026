...
Channel title resolved from migration service message (msg id=1): "Морской
48" — i.e. the informal/alternate name for бульвар Комсомольский, matching
the dual-naming pattern already on spine (pid=10718 "Б-Р КОМСОМОЛЬСКИЙ
(МОРСКОЙ), 24/13"; pid=10724 "...38"). Subject building: бульвар
Комсомольский, 48 = pid=5071, rd4u=A3.1,A3.6, 58 registry_inclusion
...
def main() -> None:
    ...
    # ── Summary ───────────────────────────────────────────────────────────────
    print(f"\n{'='*70}")
    print(f"@morskoy_48 (бульвар Комсомольский / Морской, 48, pid={PID})  —  {len(rows)} messages parsed")
