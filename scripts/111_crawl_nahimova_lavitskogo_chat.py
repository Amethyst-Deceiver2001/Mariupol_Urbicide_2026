#!/usr/bin/env python3
"""Crawl the public Telegram chat "@nahimova_lavitskogo" — full history.

Public channel: https://t.me/nahimova_lavitskogo

Likely covers the пр. Нахимова / ул. Лавицкого intersection area — both
streets are heavily represented on the spine (пр. Нахимова has ~80 entries
incl. pid=4570/Нахимова 190 and pid=5865/Нахимова 82, both already
corroboration targets; ул. Лавицкого has ~20 entries, all rd4u=A3.1 or
A3.1,A3.6/A3.1,A3.3,A3.6). This is likely a multi-building area chat, not a
single pid — resolve scope from message content (and the channel-migration
service message, if present) after capture.

NOTE on membership: if your join request to this group is still pending
approval, Telethon may not be able to read message history yet (this
differs from a read-only public broadcast channel, which doesn't require
membership at all). This script attempts the crawl regardless and reports
a clear access error if blocked, rather than assuming either way.
...
Claude must never run this. Run from project root:
    .venv312/bin/python scripts/111_crawl_nahimova_lavitskogo_chat.py
...
CHANNEL      = "nahimova_lavitskogo"
CHANNEL_SLUG = "nahimova_lavitskogo"
BUILDING_NOTE = (
    "Channel '@nahimova_lavitskogo', Mariupol. Likely covers the пр. "
    "Нахимова / ул. Лавицкого intersection area — multi-building scope, NOT "
    "a single pid. пр. Нахимова has ~80 spine entries incl. pid=4570(д.190) "
    "and pid=5865(д.82); ул. Лавицкого has ~20 entries. Resolve exact scope "
    "from message content after capture."
)
...
def _capture_message(client, con, message) -> bool:
    ...
    sha = forensics.capture_source(
        ...
        title=f"nahimova_lavitskogo/{message.id}",
        description=(
            f"Chat post nahimova_lavitskogo/{message.id} "
    ...
    forensics.capture_source(
        ...
        title=f"nahimova_lavitskogo/{message.id} media",
        description=(
            f"Media attached to nahimova_lavitskogo/{message.id} "
...
def run() -> None:
    ...
    try:
        try:
            ...
            log.error(
                "cannot access @%s yet (%s: %s) — this usually means your "
                "join request hasn't been approved. Wait for approval and "
                "re-run; a read-only public broadcast channel would not "
                "raise this, so this confirms it's a group with admin "
                "approval gating, not a broadcast channel.",
        ...
        try:
            ...
            log.info("total this run — %d messages (%d media); %d skipped post-%s",
                ...
                UserNotParticipantError) as e:
            log.error(
                "access was allowed to resolve the entity but reading "
                "messages failed (%s: %s) — join request likely still "
                "pending; re-run after approval", type(e).__name__, e,
            )
            return
    finally:
        client.disconnect()
