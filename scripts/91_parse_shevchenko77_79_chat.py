#!/usr/bin/env python3
"""Parse captured "Б-р Шевченко 77, 79" Telegram messages.

Captured under source_type='telegram_building_chat_msg', URL prefix
https://t.me/invite_LXDgwK2VGAE4MWVh/ (chat resolved from invite
https://t.me/+LXDgwK2VGAE4MWVh; title recovered from the channel-migration
service message: "Б-р Шевченко 77, 79").

Two buildings on spine, DIFFERENT tracks:
  бул. Шевченко, 77  pid=4401  registry_inclusion (title-stripping, NOT demolished)
  бул. Шевченко, 79  pid=4402  demolition event 2022-09-29

This is a mixed-track chat — useful for comparing how residents of an
ownerless-registry building and a demolished neighbor narrate the same
period differently.

Outputs:
  data/parsed/shevchenko77_79_chat_signals.jsonl
  Console summary

Run:
    python scripts/91_parse_shevchenko77_79_chat.py
"""
import json
import logging
import re
...
OUT          = ROOT / "data" / "parsed" / "shevchenko77_79_chat_signals.jsonl"
CHANNEL_SLUG = "invite_LXDgwK2VGAE4MWVh"
DEMOLITION_DATE_79 = "2022-09-29"
...
BLDG_NUM = re.compile(r"\b(?:д\.?\s*|дом\s*)(77|79)\b", re.I)
...
def main() -> None:
    ...
    # ── Summary ───────────────────────────────────────────────────────────────
    print(f"\n{'='*70}")
    print(f"Б-р Шевченко 77 (pid=4401, registry_inclusion) / 79 (pid=4402, demolished {DEMOLITION_DATE_79})")
    print(f"  {len(rows)} messages parsed")
    print(f"{'='*70}")

    print("\n── Message volume by month ──")
    for ym in sorted(month_counter):
        bar = "█" * (month_counter[ym] // 10)
        flag = "  <-- д.79 demolition order month" if ym == DEMOLITION_DATE_79[:7] else ""
