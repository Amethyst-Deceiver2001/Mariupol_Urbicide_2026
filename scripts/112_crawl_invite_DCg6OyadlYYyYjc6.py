#!/usr/bin/env python3
"""Crawl the private Telegram chat at invite https://t.me/+DCg6OyadlYYyYjc6.

Building identity unknown at script-authoring time — this invite hash has not
been resolved yet. Same pattern as scripts 89/93/101/103/105/107/109/110
(invite hashes resolved to "Б-р Шевченко 77, 79", "Меотиды 15/20", "Группа
БАХЧИВАНДЖИ 13-17" via the channel-migration service message). After this
chat is captured, check msg id=1 (or the earliest MessageService with
action._=="MessageActionChannelMigrateFrom") for the group's original
title — that's usually the building name.
...
Date window capped at 2022-12-31 per user direction (later traffic is
mostly garbage / not of evidentiary interest for this chat).

Claude must never run this. Run from project root:
    .venv312/bin/python scripts/112_crawl_invite_DCg6OyadlYYyYjc6.py
...
INVITE_HASH  = "DCg6OyadlYYyYjc6"
CHANNEL_SLUG = "invite_DCg6OyadlYYyYjc6"  # placeholder — rename once building is identified
BUILDING_NOTE = (
    "Residents' chat resolved from invite https://t.me/+DCg6OyadlYYyYjc6. "
    ...
    "first capture to determine the subject building(s). Crawl capped at "
    "2022-12-31 per user direction (later traffic is mostly garbage)."
)

DATE_FROM = dt.datetime(2018, 1, 1, tzinfo=dt.timezone.utc)
DATE_TO   = dt.datetime(2022, 12, 31, 23, 59, 59, tzinfo=dt.timezone.utc)
