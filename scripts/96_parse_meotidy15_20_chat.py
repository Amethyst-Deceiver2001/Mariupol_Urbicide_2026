#!/usr/bin/env python3
"""Parse captured "Меотиды 15/20" Telegram messages.

Captured under source_type='telegram_building_chat_msg', URL prefix
https://t.me/invite_QaRRTdUZFw0OTU6/ (chat resolved from invite
https://t.me/+Qa-RRTdUZFw0OTU6; title recovered from the channel-migration
service message, msg id=1: "Меотиды 15/20").

SPINE GAP: no exact match for "Меотиды, 15" or "Меотиды, 20" in
property.occupation_address. Only б-р 50 лет Октября (Меотиды), 40/41
(pid=10704) and 71 (pid=10707) are loaded — both rd4u=A3.3,A3.6. д.15 and
д.20 are not yet on the spine; this run is unmapped to a pid pending that
gap being filled (check the demolition register / ownerless registry for
"Меотиды" or "50 лет Октября" entries at 15/20).

Outputs:
  data/parsed/meotidy15_20_chat_signals.jsonl
  Console summary

Run:
    python scripts/96_parse_meotidy15_20_chat.py
"""
import json
import logging
import re
...
OUT          = ROOT / "data" / "parsed" / "meotidy15_20_chat_signals.jsonl"
CHANNEL_SLUG = "invite_QaRRTdUZFw0OTU6"
...
BLDG_NUM = re.compile(r"\b(?:д\.?\s*|дом\s*)(15|20)\b", re.I)
...
def main() -> None:
    ...
    # ── Summary ───────────────────────────────────────────────────────────────
    print(f"\n{'='*70}")
    print(f"Меотиды 15/20  —  {len(rows)} messages parsed")
    print("SPINE GAP: pid for д.15/д.20 not yet loaded (only д.40/41, д.71 exist)")
    print(f"{'='*70}")
