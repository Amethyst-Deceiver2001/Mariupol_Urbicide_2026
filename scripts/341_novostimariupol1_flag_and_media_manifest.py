#!/usr/bin/env python3
"""Flag+manifest pass over the @novosti_mariupol1 full-channel scan
(scripts/336), same shape and same reusable signal taxonomy as scripts/225
(@mariupol_nash) and scripts/309 (@mariupolRIP) -- see
docs/mariupol_channel_research_terms.md for the shared pattern bank this
mirrors.

Read-only, local, no network. Requires scripts/336 to have already run
(needs the "telegram_novostimariupol1_msg" raw store populated).

Emits:
  1. data/parsed/novostimariupol1_flagged_messages.jsonl -- every message that hit
     at least one strong signal (or is on the curated LEADS list), tagged.
  2. data/parsed/novostimariupol1_media_pull_manifest.jsonl -- the subset worth a
...
call. It is only useful after scripts/336 has been run by the user.

Run:
    PYTHONPATH=src python scripts/341_novostimariupol1_flag_and_media_manifest.py
"""
...
OUT_FLAGGED = ROOT / "data" / "parsed" / "novostimariupol1_flagged_messages.jsonl"
OUT_MANIFEST = ROOT / "data" / "parsed" / "novostimariupol1_media_pull_manifest.jsonl"
...
def main() -> None:
    ...
    if not rows:
        log.error("no %s rows found — run scripts/336 first (VPS/user terminal)", SOURCE_TYPE)
        sys.exit(1)
    ...
    print(f"\n{'='*72}")
    print(f"@novosti_mariupol1 FLAGGING — {n_msg} messages scanned, {flagged} flagged")
    print(f"{'='*72}")
