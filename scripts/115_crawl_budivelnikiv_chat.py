#!/usr/bin/env python3
"""Crawl the public Telegram chat "@budivelnikiv" — full history.

Public channel: https://t.me/budivelnikiv

Slug "budivelnikiv" is a transliteration of Ukrainian "будівельників"
(genitive plural of "builders") — i.e. вулиця/проспект Будівельників, the
Ukrainian name for what the spine already tracks as пр. Строителей. Several
Строителей buildings are already covered by other chats this session
(@stroiteley_175_177_163_171_166_152, scripts 82/83; Case 7 demolish-rebuild
74/76/78). No house numbers in this slug, so this may be a wider corridor
channel, not a single-building chat — do not assume a specific pid.
...
profile message content (street/building mentions) to determine scope and
avoid overlap with already-captured Строителей chats.
...
Claude must never run this. Run from project root:
    .venv312/bin/python scripts/115_crawl_budivelnikiv_chat.py
...
CHANNEL      = "budivelnikiv"
CHANNEL_SLUG = "budivelnikiv"
BUILDING_NOTE = (
    "Channel '@budivelnikiv', Mariupol. Slug transliterates Ukrainian "
    "'будівельників' = пр. Строителей. Scope NOT YET CONFIRMED — no house "
    "numbers in slug; may overlap with @stroiteley_175_177_163_171_166_152 "
    "(scripts 82/83) or other Строителей addresses. Resolve scope and check "
    "for overlap with existing captures from message content after capture."
)
...
def _capture_message(client, con, message) -> bool:
    ...
    sha = forensics.capture_source(
        ...
        title=f"budivelnikiv/{message.id}",
        description=(
            f"Chat post budivelnikiv/{message.id} "
    ...
    forensics.capture_source(
        ...
        title=f"budivelnikiv/{message.id} media",
        description=(
            f"Media attached to budivelnikiv/{message.id} "
