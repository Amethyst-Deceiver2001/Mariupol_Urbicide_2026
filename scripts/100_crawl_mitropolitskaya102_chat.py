#!/usr/bin/env python3
"""Crawl the public Telegram chat "@Mitropolitskaya102" — up to 2023-02-28.

Public channel: https://t.me/Mitropolitskaya102

Building identity NOT YET CONFIRMED on the spine. No exact "Митрополитская,
102" match exists in property.occupation_address. Closest candidates
(dual-naming pattern: ул. К.Либкнехта / Митрополитская):
...
action._=="MessageActionChannelMigrateFrom", action.title) for the
authoritative building title, the same way "Б-р Шевченко 77, 79" and
"Морской 48" were resolved for other chats.

Date window capped at 2023-02-28 per user direction (later traffic not of
evidentiary interest for this chat).

Claude must never run this. Run from project root:
    .venv312/bin/python scripts/100_crawl_mitropolitskaya102_chat.py
...
CHANNEL      = "Mitropolitskaya102"
CHANNEL_SLUG = "Mitropolitskaya102"
BUILDING_NOTE = (
    "Residents' chat '@Mitropolitskaya102', Mariupol. Building/pid NOT YET "
    "CONFIRMED — no exact 'Митрополитская, 102' match on spine. Closest "
    "candidates: pid=13982(ул.Митрополитская,98) pid=10654/10655/10656 "
    "(ул.К.Либкнехта/Митрополитская dual-naming, д.1/23/31). Resolve via "
    "channel-migration service message title after capture. Crawl capped "
    "at 2023-02-28 per user direction."
)

DATE_FROM = dt.datetime(2018, 1, 1, tzinfo=dt.timezone.utc)
DATE_TO   = dt.datetime(2023, 2, 28, 23, 59, 59, tzinfo=dt.timezone.utc)
...
def _capture_message(client, con, message) -> bool:
    ...
    sha = forensics.capture_source(
        ...
        title=f"Mitropolitskaya102/{message.id}",
        description=(
            f"Resident chat post Mitropolitskaya102/{message.id} "
    ...
    forensics.capture_source(
        ...
        title=f"Mitropolitskaya102/{message.id} media",
        description=(
            f"Media attached to Mitropolitskaya102/{message.id} "
