#!/usr/bin/env python3
"""Pull media for ONLY the flagged @kadryVoynyMariypol2022 messages -- never
the whole channel. Same shape as scripts/226/310 (the @mariupol_nash /
@mariupolRIP pullers), pointed at the manifest scripts/318 built from the
full channel scan (scripts/317).

Consumes data/parsed/kadryvoyny_media_pull_manifest.jsonl and downloads
media for those specific message ids, keyed by pull_priority:
    1 = curated lead (msg 854 -- the Зелинского cluster walkthrough video)
    2 = core seizure/demolition signal
    3 = weaker/broad testimony signal

Everything captured lands in the forensic store as source_type
"telegram_kadryvoyny_media", chain-of-custody identical to every other
...
terminal (CLAUDE.md).
    ...
    .venv312/bin/python scripts/319_pull_kadryvoyny_flagged_media.py                  # P1+P2 (default), skip video >50MB
    .venv312/bin/python scripts/319_pull_kadryvoyny_flagged_media.py --max-priority 1 # only msg 854
    .venv312/bin/python scripts/319_pull_kadryvoyny_flagged_media.py --max-priority 3 # everything in the manifest
    .venv312/bin/python scripts/319_pull_kadryvoyny_flagged_media.py --max-video-mb 0 # photos/documents only, no video
...
SOURCE_TYPE = "telegram_kadryvoyny_media"
MANIFEST = ROOT / "data" / "parsed" / "kadryvoyny_media_pull_manifest.jsonl"
BATCH = 100


def _load_manifest(manifest_path: Path, max_priority: int, max_video_mb: float) -> list[dict]:
    if not manifest_path.exists():
        log.error("manifest not found: %s — run scripts/318 first", manifest_path)
        sys.exit(1)
...
def main() -> None:
    ...
    ap.add_argument("--max-priority", type=int, default=2, choices=[1, 2, 3],
                    help="pull manifest rows up to this priority. Default 2.")
        ...
        for i in range(0, len(ids), BATCH):
            ...
            for msg in messages:
                ...
                forensics.capture_source(
                    ...
                    description=(
                        f"Flagged @{CHANNEL} media, priority "
                        f"{meta.get('pull_priority', '?')}. tags={meta.get('tags')} "
                        f"lead_note={meta.get('lead_note') or '-'}. "
