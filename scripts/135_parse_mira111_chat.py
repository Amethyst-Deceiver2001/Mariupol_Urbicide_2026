#!/usr/bin/env python3
"""Parse captured "Мира 111" Telegram messages.

Title recovered from channel-migration service message. "Мира" is the
occupation renaming of пр. Ленина -> пр. Ленина, 111/111а.
  pid=4424  Ленина, 111   no seizure events yet (gap)
  pid=4425  Ленина, 111а  registry_inclusion

Run:
    python scripts/135_parse_mira111_chat.py
"""
...
OUT          = ROOT / "data" / "parsed" / "mira111_chat_signals.jsonl"
CHANNEL_SLUG = "invite_jWgnL94OdmYmMy"
PIDS         = {"111": 4424, "111а": 4425}
...
NEW_BUILD = re.compile(r"стройк|новостройк|застройщик|новый дом|строят|строится|фундамент|кран стоит|забор поставили|огородили|плиты везут|порфир|ПОРФИР|югстройинвест|проект инвест|эволюция|мираполис|мирастрой", re.I)
...
def main():
    ...
    print(f"\n{'='*70}\nМира 111 (пр. Ленина, 111/111а)  —  {len(rows)} messages parsed\n{'='*70}")
