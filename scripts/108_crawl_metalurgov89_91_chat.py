#!/usr/bin/env python3
"""Crawl the public Telegram chat "@metalurgov89_91" — up to 2024-05-24.

Public channel: https://t.me/metalurgov89_91

Two buildings on the property spine, both demolished:
  пр. Металлургов, 89  pid=4550  rd4u=A3.1,A3.3,A3.6  demolition event on file
  пр. Металлургов, 91  pid=4551  rd4u=A3.1,A3.3,A3.6  demolition event on file

Date window capped at 2024-05-24 per user direction (later traffic not of
evidentiary interest for this chat).

Claude must never run this. Run from project root:
    .venv312/bin/python scripts/108_crawl_metalurgov89_91_chat.py
...
CHANNEL      = "metalurgov89_91"
CHANNEL_SLUG = "metalurgov89_91"
BUILDING_NOTE = (
    "Residents' chat '@metalurgov89_91', Mariupol. пр. Металлургов, 89 "
    "pid=4550, пр. Металлургов, 91 pid=4551, both rd4u=A3.1,A3.3,A3.6 with a "
    "demolition event on file. Demolished track. Crawl capped at 2024-05-24 "
    "per user direction."
)

DATE_FROM = dt.datetime(2018, 1, 1, tzinfo=dt.timezone.utc)
DATE_TO   = dt.datetime(2024, 5, 24, 23, 59, 59, tzinfo=dt.timezone.utc)
...
def _capture_message(client, con, message) -> bool:
    ...
    sha = forensics.capture_source(
        ...
        title=f"metalurgov89_91/{message.id}",
        description=(
            f"Resident chat post metalurgov89_91/{message.id} "
    ...
    forensics.capture_source(
        ...
        title=f"metalurgov89_91/{message.id} media",
        description=(
            f"Media attached to metalurgov89_91/{message.id} "
