#!/usr/bin/env python3
"""Parse captured "@Lenina106_Mariupol" Telegram messages.

One building on the spine — pid=4419 (пр. Ленина, 106), mixed-status:
1 demolition event (2022-09-29) AND 69 apartment-level registry_inclusion
(ownerless) events. Highest registry_inclusion count of any single building
crawled so far — strong candidate for an apartment-number cross-reference
(resident chat mention vs. registry designation date).

Outputs:
  data/parsed/lenina106_chat_signals.jsonl
  Console summary, incl. apartment cross-reference vs registry_inclusion
  events for pid=4419.

Run:
    python scripts/94_parse_lenina106_chat.py
"""
import json
import logging
import re
...
OUT          = ROOT / "data" / "parsed" / "lenina106_chat_signals.jsonl"
...
def _load_ownerless_events(pg_dsn: str | None):
    """Load registry_inclusion events for pid=4419 keyed by apt_raw."""
    if not pg_dsn:
        return {}
    try:
        import psycopg2
...
def main() -> None:
    ...
    # ── Summary ───────────────────────────────────────────────────────────────
    print(f"\n{'='*70}")
    print(f"@Lenina106_Mariupol (пр. Ленина, 106, pid={PID})  —  {len(rows)} messages parsed")
    print(f"Demolition order date: {DEMOLITION_DATE}; 69 registry_inclusion events on file")
    print(f"{'='*70}")
    ...
    if apt_hits:
        print(f"\n{'─'*70}")
        print(f"APARTMENT CROSS-REFERENCE: chat mention × registry_inclusion  ({len(apt_hits)} hits)")
        ...
        print("\n  (Apartment cross-reference: no hits — check DATABASE_URL or apt_raw key)")
