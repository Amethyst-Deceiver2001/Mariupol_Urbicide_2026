#!/usr/bin/env python3
"""Crawl the private Telegram chat at invite https://t.me/+Qa-RRTdUZFw0OTU6.

Building identity unknown at script-authoring time — this invite hash has not
been resolved yet. Same pattern as script 89 (https://t.me/+LXDgwK2VGAE4MWVh,
which resolved to "Б-р Шевченко 77, 79" via the channel-migration service
message). After this chat is captured, check msg id=1 (or the earliest
MessageService with action._=="MessageActionChannelMigrateFrom") for the
group's original title — that's usually the building name.
...
Date window is left wide open (2018-01-01 → today) since the chat's active
period is unknown; narrow DATE_FROM/DATE_TO once the volume-by-month profile
is visible in a first pass.

Claude must never run this. Run from project root:
    .venv312/bin/python scripts/93_crawl_invite_QaRRTdUZFw0OTU6.py
...
INVITE_HASH  = "Qa-RRTdUZFw0OTU6"
CHANNEL_SLUG = "invite_QaRRTdUZFw0OTU6"  # placeholder — rename once building is identified
BUILDING_NOTE = (
    "Residents' chat resolved from invite https://t.me/+Qa-RRTdUZFw0OTU6. "
