#!/usr/bin/env python3
"""Flag+manifest pass over the @NickolayOsychenko full-channel scan
(scripts/339), same shape and same reusable signal taxonomy as scripts/225
(@mariupol_nash) and scripts/309 (@mariupolRIP) -- see
docs/mariupol_channel_research_terms.md for the shared pattern bank this
mirrors.

Read-only, local, no network. Requires scripts/339 to have already run
(needs the "telegram_nickolayosychenko_msg" raw store populated).

Emits:
  1. data/parsed/nickolayosychenko_flagged_messages.jsonl -- every message that hit
     at least one strong signal (or is on the curated LEADS list), tagged.
  2. data/parsed/nickolayosychenko_media_pull_manifest.jsonl -- the subset worth a
...
call. It is only useful after scripts/339 has been run by the user.

Run:
    PYTHONPATH=src python scripts/344_nickolayosychenko_flag_and_media_manifest.py
"""
...
OUT_FLAGGED = ROOT / "data" / "parsed" / "nickolayosychenko_flagged_messages.jsonl"
OUT_MANIFEST = ROOT / "data" / "parsed" / "nickolayosychenko_media_pull_manifest.jsonl"
...
def main() -> None:
    ...
    if not rows:
        log.error("no %s rows found — run scripts/339 first (VPS/user terminal)", SOURCE_TYPE)
        sys.exit(1)
    ...
    print(f"\n{'='*72}")
    print(f"@NickolayOsychenko FLAGGING — {n_msg} messages scanned, {flagged} flagged")
    print(f"{'='*72}")
