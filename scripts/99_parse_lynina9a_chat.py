#!/usr/bin/env python3
"""Parse captured "@Lynina9A_Mariupol" Telegram messages.

Channel slug "Lynina" matched as пр. Лунина, 9а = pid=5816, rd4u=A3.1,A3.6,
76 registry_inclusion (apartment-level ownerless) events on file — one of
the highest single-building event counts crawled so far.

Outputs:
  data/parsed/lynina9a_chat_signals.jsonl
  Console summary, incl. apartment cross-reference vs registry_inclusion
  events for pid=5816.

Run:
    python scripts/99_parse_lynina9a_chat.py
"""
import json
import logging
import re
...
OUT          = ROOT / "data" / "parsed" / "lynina9a_chat_signals.jsonl"
...
def _load_ownerless_events(pg_dsn: str | None):
    """Load registry_inclusion events for pid=5816 keyed by apt_raw."""
    if not pg_dsn:
        return {}
    try:
        import psycopg2
...
def main() -> None:
    ...
    # ── Summary ───────────────────────────────────────────────────────────────
    print(f"\n{'='*70}")
    print(f"@Lynina9A_Mariupol (пр. Лунина, 9а, pid={PID})  —  {len(rows)} messages parsed")
    print(f"76 registry_inclusion events on file")
    print(f"{'='*70}")
