#!/usr/bin/env python3
"""Flag+manifest pass over the @kadryVoynyMariypol2022 full-channel scan
(scripts/317), same shape and same reusable signal taxonomy as scripts/225
(@mariupol_nash) and scripts/309 (@mariupolRIP) -- see
docs/mariupol_channel_research_terms.md for the shared pattern bank this
mirrors.

Read-only, local, no network. Requires scripts/317 to have already run
(needs the "telegram_kadryvoyny_msg" raw store populated).

Emits:
  1. data/parsed/kadryvoyny_flagged_messages.jsonl -- every message that hit
     at least one strong signal (or is on the curated LEADS list), tagged.
  2. data/parsed/kadryvoyny_media_pull_manifest.jsonl -- the subset worth a
     targeted media pull (scripts/319), by priority.
  3. console summary.

Claude MAY run this directly -- read-only, local, no geoblocked network
call. It is only useful after scripts/317 has been run by the user.

Run:
    PYTHONPATH=src python scripts/318_kadryvoyny_flag_and_media_manifest.py
"""
...
OUT_FLAGGED = ROOT / "data" / "parsed" / "kadryvoyny_flagged_messages.jsonl"
OUT_MANIFEST = ROOT / "data" / "parsed" / "kadryvoyny_media_pull_manifest.jsonl"
...
# ── CURATED LEADS ───────────────────────────────────────────────────────────
LEADS = {
    "854": ("video walkthrough, 2:44-4:25 -- flagged 2026-07-15 by user: "
            "narrator identifies Зелинского 17А by its green fence, among "
            "other addresses in the same cluster (17Б, 19Б, Бахчиванджи "
            "25/27, decree №178, ЖК «Нахимовский»). See "
...
def main() -> None:
    ...
    if not rows:
        log.error("no %s rows found — run scripts/317 first (VPS/user terminal)", SOURCE_TYPE)
        sys.exit(1)
    ...
    print(f"\n{'='*72}")
    print(f"@kadryVoynyMariypol2022 FLAGGING — {n_msg} messages scanned, {flagged} flagged")
    print(f"{'='*72}")
