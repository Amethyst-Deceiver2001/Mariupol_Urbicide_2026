#!/usr/bin/env python3
"""Crawl the public Telegram chat "@shevchenko74mariupol" — full history.

Public channel: https://t.me/shevchenko74mariupol

One building on the property spine:
  бул. Шевченко, 74  pid=4399  rd4u=A3.1,A3.3,A3.6  demolition event on file

Demolished track. Note: a separate chat for "Б-р Шевченко 77, 79"
(pid=4401/4402) was already captured and parsed this session (scripts
89/91, via invite +LXDgwK2VGAE4MWVh) — this is a different, nearby building
on the same boulevard, not a duplicate.
...
Claude must never run this. Run from project root:
    .venv312/bin/python scripts/116_crawl_shevchenko74_chat.py
...
CHANNEL      = "shevchenko74mariupol"
CHANNEL_SLUG = "shevchenko74mariupol"
BUILDING_NOTE = (
    "Residents' chat '@shevchenko74mariupol', Mariupol. бул. Шевченко, 74 "
    "pid=4399, rd4u=A3.1,A3.3,A3.6, demolition event on file. Demolished "
    "track. Distinct from the separately-captured 'Б-р Шевченко 77, 79' "
    "chat (pid=4401/4402, scripts 89/91) — same boulevard, different "
    "building, not a duplicate."
)
...
def _capture_message(client, con, message) -> bool:
    ...
    sha = forensics.capture_source(
        ...
        title=f"shevchenko74mariupol/{message.id}",
        description=(
            f"Resident chat post shevchenko74mariupol/{message.id} "
    ...
    forensics.capture_source(
        ...
        title=f"shevchenko74mariupol/{message.id} media",
        description=(
            f"Media attached to shevchenko74mariupol/{message.id} "
