#!/usr/bin/env python3
"""Crawl vertikal-ug.ru — ООО "Вертикаль ЮГ" (Rostov-on-Don based).

Multi-region construction company (25+ projects, Crimea + Mariupol). One
Mariupol project: ЖК "Левобережный", пр-т Тульский, 158Д/158Е.

Already corroborated via ЕИСЖС (script 72) on pid=20578:
  RPD 93-000032 "ЖК Левобережный 158Е" -- developer СЗ ВЕРТИКАЛЬ НР-1,
    ИНН 9310014722, 141 flats, 13 floors
  RPD 93-000039 "ЖК Левобережный 158Д" -- same developer, 175 flats,
    16 floors
Neither ЕИСЖС record has a land cadastral number (land_order_cadastrals:
None in both) -- that's the priority gap this crawl should try to close via
the site's own project declarations.

Site nav includes /Documentation -- highest-priority page for declaration
PDFs and the developer's own-stated ИНН/ОГРН (independent corroboration of
the ЕИСЖС-sourced 9310014722).

Same capture pattern as scripts 86/120/121/124/126/127/128: SHA-256 +
...
Claude must never run this. Run from project root:
    .venv312/bin/python scripts/129_crawl_vertikal_ug_site.py
    .venv312/bin/python scripts/129_crawl_vertikal_ug_site.py --max-pages 200
"""
import argparse
import logging
import sys
...
DOMAIN     = "vertikal-ug.ru"
START_URLS = [f"https://{DOMAIN}/"]
SOURCE_TYPE_PAGE  = "vertikal_ug_page"
SOURCE_TYPE_PDF   = "vertikal_ug_pdf"
SOURCE_TYPE_ASSET = "vertikal_ug_asset"
USER_AGENT = (
...
def run(max_pages: int) -> None:
    ...
    while queue and n_pages < max_pages:
        ...
        if url.lower().endswith(".pdf") or "application/pdf" in content_type:
            try:
                forensics.capture_source(
                    ...
                    title=f"vertikal-ug.ru PDF: {parsed.path}",
        ...
        if "text/html" not in content_type:
            try:
                forensics.capture_source(
                    ...
                    title=f"vertikal-ug.ru asset: {parsed.path}",
        ...
        try:
            forensics.capture_source(
                ...
                title=f"vertikal-ug.ru page: {parsed.path or '/'}",
    ...
    print(f"\n{'='*60}")
    print("vertikal-ug.ru crawl complete")
