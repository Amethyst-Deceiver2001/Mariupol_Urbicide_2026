#!/usr/bin/env python3
"""Parse captured "Строителей ( Будівельників )" Telegram messages.

Ukrainian-named general corridor chat for пр. Строителей, no specific house numbers in the title. Only 168 messages captured -- small chat. Parsed without pid cross-reference.

Run:
    python scripts/144_parse_stroiteley_corridor_chat.py
"""
...
OUT          = ROOT / "data" / "parsed" / "stroiteley_corridor_chat_signals.jsonl"
...
def main():
    ...
    print(f"\n{'='*70}\n'Строителей ( Будівельників )'  —  {len(rows)} messages parsed\n{'='*70}")
