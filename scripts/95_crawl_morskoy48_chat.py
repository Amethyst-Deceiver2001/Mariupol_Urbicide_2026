#!/usr/bin/env python3
"""Crawl the public Telegram chat "@morskoy_48" — full history.

Public channel: https://t.me/morskoy_48

Building identity NOT YET CONFIRMED on the spine. Likely "бульвар Морской,
48", but no exact-address match exists in property.occupation_address.
Closest candidates currently on spine (none RD4U-categorized yet, i.e. not
yet processed through script 36):
  pid=6168  бульвар Морской, 8
  pid=6182  бульвар Морской, 9
  pid=6183  бульвар Морской, 52а
  pid=6184  бульвар Морской, 44а

After capture, check the channel-migration service message (msg id=1,
action._=="MessageActionChannelMigrateFrom", action.title) the same way
"Б-р Шевченко 77, 79" was recovered for the +LXDgwK2VGAE4MWVh invite — that
should give the authoritative building name/title to resolve against the
spine (or to add as a new property if д.48 genuinely isn't loaded yet).

No date cap given for this chat — pull full history. If later triage shows
a spam-only tail (as with @kuprina33), narrow DATE_TO in a follow-up run.

Claude must never run this. Run from project root:
    .venv312/bin/python scripts/95_crawl_morskoy48_chat.py
...
BUILDING_NOTE = (
    "Residents' chat '@morskoy_48', Mariupol. Building/pid NOT YET CONFIRMED — "
    "no exact 'Морской, 48' match on spine. Closest unprocessed candidates: "
    "pid=6168(д.8) pid=6182(д.9) pid=6183(д.52а) pid=6184(д.44а). Resolve via "
    "channel-migration service message title after capture."
)
...
def _capture_message(client, con, message) -> bool:
    ...
    sha = forensics.capture_source(
        ...
        title=f"morskoy_48/{message.id}",
        description=(
            f"Resident chat post morskoy_48/{message.id} "
    ...
    forensics.capture_source(
        ...
        title=f"morskoy_48/{message.id} media",
        description=(
            f"Media attached to morskoy_48/{message.id} "
