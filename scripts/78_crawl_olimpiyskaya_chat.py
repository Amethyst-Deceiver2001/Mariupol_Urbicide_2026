#!/usr/bin/env python3
"""Crawl the private Telegram chat "Мариуполь, Олимпийская, д.71,73,75,77,79".

Private group (invite https://t.me/+uhT9ifhASZ85N2Y6, internal id 1795572318).
Account is already a member.

Five buildings on spine — all in the ownerless-registry (title-stripping) track,
NOT demolished. High event counts on д.77 (96 events) and д.79 (76 events).
A 17,487-message resident chat is the primary evidence these buildings were occupied.

Date window: 2022-01-01 → 2024-05-31 (nothing of evidentiary value after May 2024).

Claude must never run this. Run from project root:

    .venv312/bin/python scripts/78_crawl_olimpiyskaya_chat.py
...
INVITE_HASH  = "uhT9ifhASZ85N2Y6"
CHANNEL_SLUG = "olimpiyskaya_71_79"
BUILDING_NOTE = (
    "Residents' chat 'Мариуполь, Олимпийская д.71,73,75,77,79'. "
    "pid=5171(д.71) pid=5034(д.73) pid=5172(д.75) pid=5173(д.77,96 ownerless events) "
    "pid=5174(д.79,76 ownerless events). Title-stripping track; none demolished."
)

DATE_FROM = dt.datetime(2022, 1, 1, tzinfo=dt.timezone.utc)
DATE_TO   = dt.datetime(2024, 5, 31, 23, 59, 59, tzinfo=dt.timezone.utc)
...
def _capture_message(client, con, slug: str, message) -> bool:
    ...
    sha = forensics.capture_source(
        ...
        title=f"olimpiyskaya/{message.id}",
        description=(
            f"Resident chat post olimpiyskaya/{message.id} "
    ...
    forensics.capture_source(
        ...
        title=f"olimpiyskaya/{message.id} media",
        description=(
            f"Media attached to olimpiyskaya/{message.id} "
...
def run() -> None:
    ...
    try:
        ...
        if not isinstance(info, ChatInviteAlready):
            log.error("not a member (got %s) — join manually first", type(info).__name__)
            return
        ...
        # ── Pass 1: forward scan — pick up new messages since last run ────────
        # Iterates newest-first from DATE_TO, stops at the highest already-seen id.
        log.info("Pass 1 (forward): window %s → %s | stop at min_id=%d",
        ...
        log.info("pass1 done — %d new messages (%d media)", n, n_media)

        # ── Pass 2: gap-fill — fetch old messages below the crash point ───────
        # Triggered when a previous run crashed before reaching DATE_FROM.
        # Iterates backwards starting just before the lowest captured id.
        if captured_min > 1:
            log.info("Pass 2 (gap-fill): fetching messages with id < %d (crash recovery)",
                     captured_min)
            n2 = n2_media = 0
            for message in client.iter_messages(
                entity,
                max_id=captured_min,   # start just below the lowest captured id
            ):
                msg_date = message.date
                if msg_date and msg_date.tzinfo is None:
                    msg_date = msg_date.replace(tzinfo=dt.timezone.utc)
                if msg_date and msg_date < DATE_FROM:
                    log.info("gap-fill reached DATE_FROM at msg id=%d — done",
                             message.id)
                    break
                if _capture_message(client, con, CHANNEL_SLUG, message):
                    n2_media += 1
    ...
    for st, label in [(SOURCE_TYPE_MSG, "msg"), (SOURCE_TYPE_MEDIA, "media")]:
        cnt = con.execute("SELECT COUNT(*) FROM source_document WHERE source_type=?", (st,)).fetchone()[0]
