#!/usr/bin/env python3
"""Crawl the public Telegram chat "@mariupol_komsomolets" — full history.

Public channel: https://t.me/mariupol_komsomolets

Building identity NOT assumed — "komsomolets" likely refers to a
general-purpose Mariupol community/news channel (cf. "Mariupol Online",
"Мариуполь INFO" channels surfacing repeatedly as cross-posted sources
inside other building chats parsed this session), not necessarily a single
building's resident group. Possible link to б-р Комсомольский (dual-named
"Морской" boulevard, already on spine — see script 98/pid=5071), but this
should NOT be assumed without confirmation.

After capture, check the channel-migration service message (msg id=1,
action._=="MessageActionChannelMigrateFrom", action.title) if present — for
a long-running public news channel this may be absent (channels don't
always show a migration event); in that case profile message content
directly (street/building mentions, signal flags) to determine scope.
...
Claude must never run this. Run from project root:
    .venv312/bin/python scripts/106_crawl_mariupol_komsomolets_chat.py
...
CHANNEL      = "mariupol_komsomolets"
CHANNEL_SLUG = "mariupol_komsomolets"
BUILDING_NOTE = (
    "Channel '@mariupol_komsomolets', Mariupol. Building/scope NOT YET "
    "CONFIRMED — may be a general community/news channel rather than a "
    "single-building resident chat (cf. other cross-posted general channels "
    "like 'Mariupol Online'/'Мариуполь INFO' seen inside other building "
    "chats). Possible but unconfirmed link to б-р Комсомольский (Морской), "
    "pid=5071 and neighbors. Resolve scope from message content after capture."
)
...
def _capture_message(client, con, message) -> bool:
    ...
    sha = forensics.capture_source(
        ...
        title=f"mariupol_komsomolets/{message.id}",
        description=(
            f"Chat post mariupol_komsomolets/{message.id} "
    ...
    forensics.capture_source(
        ...
        title=f"mariupol_komsomolets/{message.id} media",
        description=(
            f"Media attached to mariupol_komsomolets/{message.id} "
