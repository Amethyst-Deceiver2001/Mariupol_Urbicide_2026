#!/usr/bin/env python3
"""Crawl the public Telegram chat "@Lenina106_Mariupol" — full history.

Public channel: https://t.me/Lenina106_Mariupol

One building on the property spine, with an unusually high event count:
  пр. Ленина, 106  pid=4419  rd4u=A3.1,A3.6  demolition (2022-09-29) AND
                              69 registry_inclusion events (apartment-level
                              ownerless designations) — a mixed-status
                              building: bulk demolition order alongside
                              individual unit-by-unit ownerless processing.
...
Claude must never run this. Run from project root:
    .venv312/bin/python scripts/92_crawl_lenina106_chat.py
...
CHANNEL      = "Lenina106_Mariupol"
CHANNEL_SLUG = "Lenina106_Mariupol"
BUILDING_NOTE = (
    "Residents' chat '@Lenina106_Mariupol', Mariupol. пр. Ленина, 106 pid=4419. "
    "Mixed-status building: 1 demolition event (2022-09-29) AND 69 "
    "registry_inclusion events (apartment-level ownerless designations) — "
    "bulk demolition order alongside unit-by-unit ownerless processing."
)
...
def _capture_message(client, con, message) -> bool:
    ...
    sha = forensics.capture_source(
        ...
        title=f"Lenina106/{message.id}",
        description=(
            f"Resident chat post Lenina106/{message.id} "
    ...
    forensics.capture_source(
        ...
        title=f"Lenina106/{message.id} media",
        description=(
            f"Media attached to Lenina106/{message.id} "
