#!/usr/bin/env python3
"""Crawl lazurnieberega.ru -- ООО "СЗ "Сириус Билд"" / ЖК "Лазурные берега".

Developer: ООО «СЗ «Сириус Билд»», ИНН 9310014320, ОГРН 1249300010607,
construction license № 93-37-031-2025 (20.03.2025). Project: ул. Куприна,
69б, completion target Q4 2026.

This is the THIRD developer project found on ул. Куприна this session
(after @kuprina33's pid=4939/д.33, and SZ "Антарес"'s pid=4946/д.65):
  pid=4947 (Куприна, 69) -- demolished, NO reallocation event yet.

Site nav includes a "ДОМ.РФ" external registry link -- useful for
cross-checking the project's official registration entry (наш.дом.рф), and
a "Расположение" (Location) page that may carry a more precise address or
neighborhood map similar to the usi-mariupol.ru/mar-s.group pattern.

Same capture pattern as scripts 86/120/121/124/126-131: SHA-256 + meta.json
via forensics.capture_source, skips static asset extensions. Can run in
parallel with other crawls.

Claude must never run this. Run from project root:
    .venv312/bin/python scripts/132_crawl_lazurnieberega_site.py
    .venv312/bin/python scripts/132_crawl_lazurnieberega_site.py --max-pages 200
"""
import argparse
import logging
import sys
...
DOMAIN     = "lazurnieberega.ru"
START_URLS = [f"https://{DOMAIN}/"]
SOURCE_TYPE_PAGE  = "lazurnieberega_page"
SOURCE_TYPE_PDF   = "lazurnieberega_pdf"
SOURCE_TYPE_ASSET = "lazurnieberega_asset"
USER_AGENT = (
...
def run(max_pages: int) -> None:
    ...
    while queue and n_pages < max_pages:
        ...
        if url.lower().endswith(".pdf") or "application/pdf" in content_type:
            try:
                forensics.capture_source(
                    ...
                    title=f"lazurnieberega.ru PDF: {parsed.path}",
        ...
        if "text/html" not in content_type:
            try:
                forensics.capture_source(
                    ...
                    title=f"lazurnieberega.ru asset: {parsed.path}",
        ...
        try:
            forensics.capture_source(
                ...
                title=f"lazurnieberega.ru page: {parsed.path or '/'}",
    ...
    print(f"\n{'='*60}")
    print("lazurnieberega.ru crawl complete")
