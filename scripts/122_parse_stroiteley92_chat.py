#!/usr/bin/env python3
"""Parse captured "СТРОИТЕЛЕЙ 92" Telegram messages.

Title recovered from the channel-migration service message (msg id=1):
"СТРОИТЕЛЕЙ 92". One building on the spine:

  пр. Строителей, 92  pid=4648  rd4u=A3.1  NO seizure events on file (gap)

~2,630 messages captured — one of the larger chats this session. No seizure
stage loaded yet for this building, so this chat is the primary evidentiary
source for whatever happened here (siege damage, demolition, or registry
inclusion) until cross-checked against the demolition/ownerless registers.

Outputs:
  data/parsed/stroiteley92_chat_signals.jsonl
  Console summary

Run:
    python scripts/122_parse_stroiteley92_chat.py
"""
import json
import logging
import re
...
OUT          = ROOT / "data" / "parsed" / "stroiteley92_chat_signals.jsonl"
CHANNEL_SLUG = "invite_ucWZaRSL1Gk1NjRi"
...
NEW_BUILD = re.compile(
    ...
    r"югстройинвест|проект инвест|нахимовск",
    re.I,
)
BURIAL = re.compile(
...
BLDG_NUM = re.compile(r"\b(?:д\.?\s*|дом\s*)(92)\b", re.I)
...
def main() -> None:
    ...
    # ── Summary ───────────────────────────────────────────────────────────────
    print(f"\n{'='*70}")
    print(f"СТРОИТЕЛЕЙ 92 (pid=4648, NO seizure events on file)  —  {len(rows)} messages parsed")
    print(f"{'='*70}")
