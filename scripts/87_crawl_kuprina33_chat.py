#!/usr/bin/env python3
"""Crawl the public Telegram chat "@kuprina33" — from inception to 2022-11-30.

Public channel: https://t.me/kuprina33

One building on the property spine:
  ул. Куприна, 33  pid=4939  rd4u=A3.1  demolition event 2022-09-29

Demolished track. Spam/noise dominates from December 2022 onward per user
direction — this run stops at 2022-11-30. Likely contains siege-era testimony
predating the September 2022 demolition (rebuts any "already vacant" framing
for the demolition predicate) plus first-hand accounts of the demolition
itself in the surrounding weeks.

Date window: from channel inception → 2022-11-30.

Claude must never run this. Run from project root:
    .venv312/bin/python scripts/87_crawl_kuprina33_chat.py
...
BUILDING_NOTE = (
    "Residents' chat '@kuprina33', Mariupol. ул. Куприна, 33 pid=4939. "
    "Demolished track (demolition event 2022-09-29). Chat capped at 2022-11-30 "
    "per user direction — later messages are spam."
)

DATE_FROM = dt.datetime(2018, 1, 1, tzinfo=dt.timezone.utc)  # generous floor; channel inception is earlier in practice
DATE_TO   = dt.datetime(2022, 11, 30, 23, 59, 59, tzinfo=dt.timezone.utc)
...
def _capture_message(client, con, message) -> bool:
    ...
    sha = forensics.capture_source(
        ...
        title=f"kuprina33/{message.id}",
        description=(
            f"Resident chat post kuprina33/{message.id} "
    ...
    forensics.capture_source(
        ...
        title=f"kuprina33/{message.id} media",
        description=(
            f"Media attached to kuprina33/{message.id} "
