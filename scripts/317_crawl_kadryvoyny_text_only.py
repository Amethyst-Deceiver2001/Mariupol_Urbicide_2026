#!/usr/bin/env python3
"""Scan @kadryVoynyMariypol2022 -- text messages only, no media downloads.

Flagged 2026-07-15 by user review of the Зелинского 17А/17Б/19Б +
Бахчиванджи 25/27 demolition cluster (ЖК «Нахимовский», decree №178, see
docs/case_studies/death_sites_new_construction.md Case 2): a video posted
to this channel (t.me/kadryVoynyMariypol2022/854, 2:44-4:25) shows the
narrator identifying буilding 17А by its green fence, among others in the
same walkthrough -- "definitely needs a scan" per the user, since the
channel had not previously been added to this project's chat corpus at all
(confirmed: not present in src/mariupol_seizures/chat_buildings.py or any
existing crawl target list).

Same text-only, resumable-by-source_type methodology as scripts/211/212/
227/234/302: skip client.download_media() entirely so a full-history run
stays small (JSON message objects are ~1-5 KB each); use a 225/309-style
flag-then-targeted-pull pass afterward (scripts/318) to recover media for
high-value leads only, never the whole channel's video backlog up front.

Source-type: "telegram_kadryvoyny_msg" (own namespace, independent
resumability bookkeeping from every other channel crawl in this project).
...
foreign-state-adjacent service. Run from your own Russia-routed terminal:
    ...
    .venv312/bin/python scripts/317_crawl_kadryvoyny_text_only.py
    .venv312/bin/python scripts/317_crawl_kadryvoyny_text_only.py --backfill
...
Next steps once this has run:
    .venv312/bin/python scripts/318_kadryvoyny_flag_and_media_manifest.py   (local, safe for Claude)
    .venv312/bin/python scripts/319_pull_kadryvoyny_flagged_media.py        (VPS-only)
"""
...
NOTE = ("«Кадры войны, Мариуполь 2022» -- flagged 2026-07-15 during review "
        "of the Зелинского/Бахчиванджи demolition-cluster video artifacts. "
        "Full-channel text scan; not previously part of this project's "
        "chat corpus. No spine property_id mapping at capture time -- "
        "resolved separately once flagged leads are reviewed. Text-only "
        "capture.")
