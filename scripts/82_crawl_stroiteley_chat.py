#!/usr/bin/env python3
"""Crawl the public Telegram chat "Строителей 175,177,171,163,166,152" — up to 2023-08-31.

Public channel: https://t.me/chat177i175i163i171i166i152
id=1502962323, 23,562 total messages, earliest 2022-03-13 (active siege period).

Six buildings on the property spine — all title-stripping track (NOT demolished):
  пр. Строителей, 152  pid=4610  rd4u=A3.1,A3.6  registry_inclusion ×2
  пр. Строителей, 163  pid=4618  rd4u=A3.1,A3.6  registry_inclusion ×3
  пр. Строителей, 166  pid=4621  rd4u=A3.1        no seizure events yet
  пр. Строителей, 171  pid=4624  rd4u=A3.1,A3.6  registry_inclusion ×2
  пр. Строителей, 175  pid=4625  rd4u=A3.1        no seizure events yet
  пр. Строителей, 177  pid=4626  rd4u=A3.1        no seizure events yet

These buildings are on the same boulevard as Case 7 (Строителей 74–88).
Chat starts mid-siege (13 Mar 2022) — likely contains damage/survival testimony
predating the ownerless registry designations, rebutting the abandonment predicate.

Date window: 2022-03-13 → 2023-08-31 (post-August 2023 is predominantly spam).

Claude must never run this. Run from project root:
    .venv312/bin/python scripts/82_crawl_stroiteley_chat.py
...
CHANNEL      = "chat177i175i163i171i166i152"
CHANNEL_SLUG = "stroiteley_175_177_163_171_166_152"
BUILDING_NOTE = (
    "Residents' chat 'Строителей 175,177,171,163,166,152', Mariupol. "
    "пр. Строителей: д.152 pid=4610, д.163 pid=4618, д.166 pid=4621, "
    "д.171 pid=4624, д.175 pid=4625, д.177 pid=4626. "
    "Title-stripping track; same boulevard as Case 7 (74-88 demolished). "
    "Chat active from siege onset (2022-03-13)."
)

DATE_FROM = dt.datetime(2022, 1, 1, tzinfo=dt.timezone.utc)
DATE_TO   = dt.datetime(2023, 8, 31, 23, 59, 59, tzinfo=dt.timezone.utc)
...
def _capture_message(client, con, message) -> bool:
    ...
    sha = forensics.capture_source(
        ...
        title=f"stroiteley/{message.id}",
        description=(
            f"Resident chat post stroiteley/{message.id} "
    ...
    forensics.capture_source(
        ...
        title=f"stroiteley/{message.id} media",
        description=(
            f"Media attached to stroiteley/{message.id} "
