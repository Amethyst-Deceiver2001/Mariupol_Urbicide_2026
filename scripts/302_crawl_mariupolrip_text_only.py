#!/usr/bin/env python3
"""Scan @mariupolRIP -- text messages only, no media downloads.

"Погибшие и Пропавшие, Мариуполь" -- the volunteer memorial channel that is
the dominant primary source behind the mariupoldestruction.com named-victims
sheet (see memory/mariupoldestruction_victims_tsv.md,
scripts/299_capture_victims_map_full_sheet.py,
scripts/300_extract_courtyard_grave_sites.py). Most "Источник информации"
cells in that sheet are individual https://t.me/mariupolRIP/NNNN links, but
this project has only ever captured those individual cited posts
(scripts/239, 240) -- never scanned the channel itself. A full-channel scan
surfaces:
  - posts documenting a death/burial that never made it into the sheet (or
    were merged/summarized away),
  - the un-cited context around a cited post (thread, replies, follow-ups --
    same pattern as the ssaniaworld/lenina133 follow-up posts that named
    officials Климов and Солодуха after a first pass only found the
    original post),
  - independent corroboration/richer detail for rows already in
    data/reports/courtyard_grave_sites.csv and
    data/reports/grave_sites_property_overlap.csv.

Same text-only methodology as scripts/211/212/227: skips
client.download_media() entirely so a full-history run stays small (JSON
message objects are ~1-5 KB each). Use scripts/225-style flag-then-targeted-
pull tooling afterward to recover media for any high-value leads this
surfaces, rather than downloading everything up front.

Source-type: "telegram_mariupolrip_msg" (own namespace, independent
resumability bookkeeping from every other channel this project has crawled,
and from the individual-post captures already in source_type
"telegram_post" via scripts/239).
...
foreign-state-adjacent service. Run from your own Russia-routed terminal:
    ...
    .venv312/bin/python scripts/302_crawl_mariupolrip_text_only.py
    .venv312/bin/python scripts/302_crawl_mariupolrip_text_only.py --backfill
...
NOTE = ("«Погибшие и Пропавшие, Мариуполь» -- volunteer-run civilian memorial "
        "channel, 2022-. Dominant primary source behind the "
        "mariupoldestruction.com named-victims sheet (scripts/299/300/301). "
        "Full-channel text scan to recover un-cited posts, follow-up/reply "
        "context, and corroboration for existing grave-site rows. No spine "
        "property_id mapping at capture time -- that join happens "
        "separately (scripts/301-style). Text-only capture.")
...
# channel crawler in this project, including the individual mariupolRIP
# posts already captured under source_type "telegram_post" (scripts/239)
# ---------------------------------------------------------------------------
