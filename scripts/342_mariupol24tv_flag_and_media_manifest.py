#!/usr/bin/env python3
"""Flag+manifest pass over the @mariupol24tv full-channel scan
(scripts/337), same shape and same reusable signal taxonomy as scripts/225
(@mariupol_nash) and scripts/309 (@mariupolRIP) -- see
docs/mariupol_channel_research_terms.md for the shared pattern bank this
mirrors.

Read-only, local, no network. Requires scripts/337 to have already run
(needs the "telegram_mariupol24tv_msg" raw store populated).

Emits:
  1. data/parsed/mariupol24tv_flagged_messages.jsonl -- every message that hit
     at least one strong signal (or is on the curated LEADS list), tagged.
  2. data/parsed/mariupol24tv_media_pull_manifest.jsonl -- the subset worth a
...
call. It is only useful after scripts/337 has been run by the user.

Run:
    PYTHONPATH=src python scripts/342_mariupol24tv_flag_and_media_manifest.py
"""
...
SOURCE_TYPE = "telegram_mariupol24tv_msg"
OUT_FLAGGED = ROOT / "data" / "parsed" / "mariupol24tv_flagged_messages.jsonl"
OUT_MANIFEST = ROOT / "data" / "parsed" / "mariupol24tv_media_pull_manifest.jsonl"
...
def main() -> None:
    ...
    if not rows:
        log.error("no %s rows found — run scripts/337 first (VPS/user terminal)", SOURCE_TYPE)
        sys.exit(1)
    ...
    print(f"\n{'='*72}")
    print(f"@mariupol24tv FLAGGING — {n_msg} messages scanned, {flagged} flagged")
    print(f"{'='*72}")
