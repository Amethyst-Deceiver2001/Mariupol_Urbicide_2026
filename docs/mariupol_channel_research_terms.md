...
| **Housing inventory campaign** | `инвентаризац*` `сплошн* инвентаризац*` `инвентариз* жилищн* фонд*` `второй этап инвентаризац*` `повторн* инвентаризац*` | Added 2026-07-08 after a run of high-value finds all surfaced this way (Распоряжение №619, the phase-2 novosti_284 registration deadline, Зелинского 13, the Орджоникидзевский-district notice) — nearly every hit is the actual door-to-door/registry-deadline mechanism, not noise. Co-occurs heavily with `бесхозя*`, personal-appearance language (`явит* лично` `паспорт* и правоустанавливающ*`), and district-office addresses. | distinguish from generic municipal "инвентаризация" of unrelated assets (roads, utility networks) — check for a housing/жиль* word nearby if the hit doesn't already mention an address |
| **Border/entry filtration screening** | `фильтрац*` `не пуска*` `не пропуст*` `не пропускают` `на границе\|погранконтрол*` | Added 2026-07-09 after two @ssaniaworld testimonies (msg 4358, 4497) surfaced this as a repeat mechanism: a claimant with RF-friendly credentials and prior clean entries (2016/18/22/23) is denied entry in 2024 after being flagged in "фильтрация" screening at Sheremetyevo/Mariupol police, with no reason given beyond a scripted "работайте дальше" — i.e. a border-level screening layer that can physically block a would-be claimant from ever reaching the personal-appearance registration step (№66-РЗ ст.5(3)(а)), independent of and prior to the citizenship/documentation gate already documented. A follow-up sweep (2026-07-09, 681 raw hits) confirmed a **strong, dominant noise pattern**: the great majority of hits (~87%, mostly dated April 2022) refer to the wartime civilian-screening checkpoints active during the siege/early occupation — a related but distinct usage from the 2024-2026 property-reclamation-blocking pattern. **Filter to 2023+ and require property-adjacent co-occurrence** (`квартир*` `жиль*` `дом*` `бесхоз*` `собственност*` `вернут*` `возвращ*` `регистрац*`) to isolate the relevant sense — this cut 681 hits to 45 candidates, of which several were genuinely new (@ssaniaworld/3341's 5,700-object figure; /4306, /4329). Worth treating as its own lifecycle-blocking mechanism, not folded into the citizenship-gate thread — a person can hold RF citizenship, a valid passport, and still be filtered out at the border. | most raw hits are the 2022-era wartime-checkpoint sense, not the property-reclamation-blocking sense — always date-filter (2023+) and require a property/return co-occurrence word before treating a hit as on-topic |
| **Распоряжение №264 (06.06.2024)** | `распоряжени\w*\s*(№\|номер\|N)?\s*264` `\bРаспоряжение 264\b` `«?264»?\s*(от\|,)` | Added 2026-07-09, actively monitored, standing open item — a photographed paper (`@mrpl_besxozxata/31910`) shows a legible letterhead matching this exact date/number/title («О проведении инвентаризации жилого фонда, расположенного на административной территории города Мариуполя»), with an internally consistent legal-citation chain (5-ФКЗ, 131-ФЗ, №2501, 218-ФЗ, №66-РЗ, приказ №П/0086) matching instruments already independently verified elsewhere in this project — see `docs/legal_mechanisms_review.md`. Not yet found on the mariupol-r897.gosweb.gosuslugi.ru site's own "инвентаризация" index, and the primary PDF/full text (operative clauses, signature block) has not been independently located. Multiple residents/buildings have independently referenced "264" on door-posted notices, so this is not a single-source artifact — worth a standing archive sweep whenever the corpus grows, and matching against `распоряжение 264` WITHOUT the № sign too, since resident chat references frequently drop it. | do not conflate with №619/№71 (a separate, earlier decree, different title) or with any other numeric coincidence (house numbers, apartment counts) — require a "Распоряжение"/"инвентаризац*" co-occurrence within the same message before treating a bare "264" as on-topic |
